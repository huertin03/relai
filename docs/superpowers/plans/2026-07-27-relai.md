...
func NewCodexProvider() CodexProvider {
	// Transport se deja a nil a propósito: si se ligara aquí a p.spawn, el
	// method value capturaría esta copia y reasignar Bin después (como hace
	// la Task 9 con binaries.codex) no tendría ningún efecto. Se resuelve en
	// Usage, con el Bin vigente en ese momento.
...
- [ ] **Step 1b: Escribir las notificaciones nativas**
...
`notify` vive en `notify.go` (Step 1b), no aquí. No importar `log` en `tray.go` si no se usa para otra cosa.
...
### Task 10: Build multiplataforma, CI y documentación
...
**Interfaces:**
- Consumes: nada nuevo. Todo el código está terminado tras la Task 9.
- Produces: `make test`, `make build`, `make cross`.
...
- [ ] **Step 2: Verificar la cross-compilación**
...
- [ ] **Step 5: Ejecutar la suite completa y commitear**

Run: `make test && make cross`
Expected: PASS y tres binarios.

```bash
git add Makefile .github/workflows/ci.yml README.md go.mod go.sum
